...
同じフォーマットのスプレッドシートが複数ある（共用と個人など）ため、**送信先**を
複数登録して切り替えられるようにしている。
...
Googleアカウントとスプレッドシートの組を「送信先」として複数持てる（`Destination`）。
1件が `{ name, apiKey, gasUrl, passphrase }`。モデルと解像度だけは送信先によらず共通。

**APIキーも送信先ごとに持たせている。** 同じキーを両方に入れてもよいが、アカウントごとに
分けると無料枠を分散できる。

切り替えは2箇所。設定画面のラジオボタン（既定の送信先）と、**送信直前の確認ダイアログ**。
共用と個人を取り違えるのは費目の間違いより厄介なので、ダイアログにも置いている。
実行ボタンには送信先名を出す（例:「個人 に 食費 / 内食 で追記」）。

解析に使うAPIキーは、**解析した時点で選ばれていた送信先**のもの。ダイアログでの切り替えは
行の書き込み先だけを変える（解析はすでに終わっているため）。

保存キーは `receipt-to-sheet:settings:v3`。単一送信先だった `v2` は読み込み時に
「移行した設定」という名前の送信先1件へ自動移行する。

### 履歴（localStorage）

直近5件の解析結果を保存する（`src/lib/history.ts`）。用途は2つあり、後者を忘れないこと。

- 過去の入力を確認・訂正する
- **開発時に毎回APIを叩かずに確認画面を再現する**

履歴から開いた場合はAPIを呼ばない。解析結果・画像・使用モデル・解像度を保存しているので、
確認画面と備考の整形結果をそのまま再現できる。JSONの生データも履歴画面から見られる。

保存に失敗したときは、古い方から画像だけを捨てて再試行する（`persist`）。履歴ごと失うより
本文を残すほうがよい。画像は1件あたり数十KB〜百数十KBで、5件でも1MB弱に収まる。

履歴から開いた確認画面で再送信すると、シートには**新しい行が追記される**。既存行の更新では
ない（シート側に行を特定する手段がないため）。
...
1. **設定** — 送信先の管理と共通設定。localStorageに保存
2. **撮影** — カメラ起動
...
6. **完了** — 送信結果の表示
7. **履歴** — 直近の解析結果を一覧。撮影画面から入る（履歴が1件以上あるときだけ表示）
